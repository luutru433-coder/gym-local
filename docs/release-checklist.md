...
5. Verify backup v5 export/restore, backup v1–v4 import, IndexedDB v4→v5 migration, meal-plan append-only/idempotent logging, pre-restore recovery/undo, active-workout refusal, and nutrition-pack preservation.
...
7. Upload the manifest-named SQLite file to its matching `nutrition-vYYYY.MM.PATCH` release, then deploy the exact verified commit.
...
9. Verify install, offline relaunch, deferred update during an active workout, schema-2 compatibility, schema-3 pack install/search/remove/update/rollback, and recovery after an interrupted pack download.
